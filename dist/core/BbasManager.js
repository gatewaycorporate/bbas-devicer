...
// ── Warning messages ──────────────────────────────────────────
const LICENSE_WARN = '[bbas-devicer] No license key — running on the free tier ' +
...
const LICENSE_INVALID_WARN = '[bbas-devicer] License key could not be validated — falling back to the free tier. ' +
    'Check your key or network connectivity.';
const DEVICE_LIMIT_WARN = `[bbas-devicer] Free-tier device limit reached (${FREE_TIER_MAX_DEVICES.toLocaleString()} devices). ` +
...
function normalizeStorage(s) {
    if ('init' in s && typeof s.init === 'function') {
        return s;
    }
    const sync = s;
    return {
 ...
 */
function extractCrossPluginSignals(details) {
    const ip = details['ip'] ?? {};
    ...
    return {
        ...
        isAiAgent: ip['agentInfo']?.isAiAgent === true,
        aiAgentProvider: ip['agentInfo']?.aiAgentProvider,
        tlsConsistencyScore: typeof tls['consistencyScore'] === 'number'
            ? tls['consistencyScore']
            : undefined,
        tlsFactors: Array.isArray(tls['factors']) ? tls['factors'] : undefined,
...
export class BbasManager {
    ...
    constructor(options = {}) {
        ...
        this.storage = normalizeStorage(options.storage ?? createBbasStorage(maxHistory));
    }
    // ── Accessors ─────────────────────────────────────────────────
    ...
    async analyze(deviceId, context, crossPlugin, behavioralMetrics) {
        ...
        if (this.opts.enableVelocity) {
            ...
            velocitySignals = computeVelocitySignals(history, this.opts.velocityWindowMs, this.opts.maxRequestsPerWindow);
        }
        else {
            velocitySignals = {
                requestCount: 0,
        ...
        const enrichmentWithoutDecision = {
            ...partialEnrichment,
            consistencyScore,
        };
        // ── Rule engine ───────────────────────────────────────────
        ...
        const decision = applyRules(enrichment, this.opts.rules, this.opts.challengeThreshold, this.opts.blockThreshold);
...
// ── Helpers ───────────────────────────────────────────────────
function flattenHeader(v) {
    if (!v)
        return undefined;
    return Array.isArray(v) ? v[0] : v;
}
function buildEmptyEnrichment(headers, windowMs) {
    return {
        botScore: 0,
        botFactors: [],
        decision: 'allow',
...
//# sourceMappingURL=BbasManager.js.map